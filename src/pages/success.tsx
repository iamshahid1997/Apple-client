...
function Success() {
  ...
  useEffect(() => {
    let total = 0;
    if (order_details) {
      order_details.getLatestOrder.orders.map(
        (order: {
          quantity: number;
          order: {
            price: number;
          };
        }) => (total = total + order.quantity * order.order.price)
      );
      setTotalPrice(total);
    }
  }, [order_details]);
            ...
            <div className='my-8 ml-4 flex space-x-4 lg:ml-14 xl:ml-0'>
              ...
              <div>
                <p className='text-sm text-gray-600'>
                  Order #{order_details.getLatestOrder.user._id}
                </p>
                <h4 className='text-lg'>
                  Thank you {order_details.getLatestOrder.user.username}
                </h4>
              </div>
            </div>
                  ...
                  <div className='flex justify-between pt-4'>
                    ...
                    <p className='flex items-center gap-x-2 text-xs text-[gray]'>
                      ...
                      <span className='text-xl font-medium text-black'>
                        <Currency quantity={totalPrice + 20} currency='INR' />
                      </span>
                    </p>
                  </div>
                </div>
